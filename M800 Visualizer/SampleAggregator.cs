// Taken from NAudio sample player: https://github.com/naudio/NAudio/blob/master/NAudioWpfDemo/AudioPlaybackDemo/SampleAggregator.cs
...
namespace M800_Visualizer
{
    public class SampleAggregator
    {
        ...
        public SampleAggregator(int bufferSize)
        {
            ...
            myFftBuffer = new float[bufferSize];
        }
        ...
        public void GetFFTResults(float[] fftBuffer)
        {
            ...
            for (int i = 0; i < channelDataClone.Length / 2; i++)
            {
                ...
                fftBuffer[i] = 10 * (float)Math.Sqrt(channelDataClone[i].X * channelDataClone[i].X + channelDataClone[i].Y * channelDataClone[i].Y);
            }
        }
